...
const menus: Record<Role, MenuItem[]> = {
  ...
  Coder: [{ label:'Tablero',slug:'tablero',icon:LayoutDashboard },{ label:'Sprint',slug:'sprint',icon:CalendarDays },{ label:'Proyectos',slug:'proyectos',icon:FolderKanban },{ label:'Ceremonias',slug:'ceremonias',icon:Sparkles },{ label:'Documentación',slug:'documentacion',icon:BookOpenText },{ label:'Evaluaciones',slug:'evaluaciones',icon:ClipboardCheck },{ label:'Mi perfil',slug:'perfil',icon:Users }],
}
...
export function RoleShell({ role, name, children }: { role: Role; name: string; children: ReactNode }) {
  ...
  const [roses, setRoses] = useState<number | null>(null)
  const searchRef = useRef<HTMLInputElement>(null)
  const location = useLocation()
  const navigate = useNavigate()
  const user = getCurrentUser()
  const displayName = user?.fullName || name
  const contextTitle = user?.clan || user?.campus || user?.cohort || '—'
  const contextDetail = [user?.cell, user?.cohort].filter(Boolean).join(' · ') || user?.campus || '—'
  ...
  useEffect(() => {
    if (!user?.id || user.role !== 'coder') return
    apiRequest<{ metrics: { roses: number } }>(`/coders/${user.id}/profile`)
      .then(data => setRoses(data.metrics.roses))
      .catch(() => undefined)
  }, [user?.id, user?.role])
  ...
  return (
    <div className={`app-shell app-shell--premium role-${roleClass} ${collapsed ? 'app-shell--collapsed' : ''}`}>
      ...
      {showNotifications && <div className="modal-backdrop" onMouseDown={() => setShowNotifications(false)}><section className="modal-card modal-card--side" onMouseDown={event => event.stopPropagation()}><p className="eyebrow">Centro de avisos</p><h2>Notificaciones</h2>{notifications.length === 0 && <p>No hay avisos activos para esta sesión.</p>}{notifications.map(item => <button type="button" className="notification-row" key={item.id} onClick={() => {
        if (!item.readAt) {
        ...
        if (item.url) { setShowNotifications(false); navigate(item.url) }
